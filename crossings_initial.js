...
function hasLinkCrossingsWithInputLink (inputLink) {
    var i, j, c = 0, link1, link2, line1, line2;;
    //var res = [];
...
function linkCrossingsParamQuadTree () {
  ...
  for(var i = 0;i<links.length;i++)
  {
    ...
    let cx = (links[i].source.x+links[i].target.x)/2;
    let cy = (links[i].source.y+links[i].target.y)/2;
    //console.log(quadtree.find(cx, cy, dis/2+10));
  }
