...
def find_suitable_point(u, v_x, v_y, D_x, D_y, edge_list):
 crossing_exist = True
 while crossing_exist:
  ...
  if not crossing_exist:
   # future todo: check whether the edge is inside a box, then run binary search, this is harder, we don't know any valide end point
   return v_x, v_y
...
def iterative_force_directed(G):
 ...
 while i<len(edge_list):
  ...
  v_x, v_y = find_suitable_point(u, v_x, v_y, D_x, D_y, edge_list)
  ...
  while max_diff>10:
   ...
   for j in range(4, len(D_x)):
    ...
    for k in range(len(D_x)):
     ...
     if not D_x[j]==D_x[k]: repulsion_x += param*((D_x[j]-D_x[k])/abs(D_x[j]-D_x[k]))/abs(D_x[j]-D_x[k])
